...
uint8_t encA, encB, dir=0;
int8_t i=0, j=0, encoder, dataCnt=0, kira=0;
uint8_t mode=0, cnt=0, flag=0;
char tx2buf[100];
...
{	if(htim->Instance==TIM6)
	{	for(uint8_t j=0;j<5;j++)
		{	latchPin(0);
			...
			else        pushDataToShiftRegister(pattern[i][j]);
			latchPin(1);
		}
	}
}
	  ...
	  {	  if(t0 < HAL_GetTick())
  	  ...
  	  {	  if(t1 < HAL_GetTick())
  		  {	  t1 += 2;
  		  	  ...
  		  	  {	  if(encoder==1)
  		  		  {	  dataCnt++;
  		  			  if(dataCnt==4)  { dataCnt=0; i++; }
  		  			  if(i>5) { i=5; beep(2,50); }
   		  		  }
  		  	  	  if(encoder==-1)
				  {	  dataCnt--;
					  if(dataCnt==-4) { dataCnt=0; i--; }
					  if(i<1) { i=1; beep(2,50); }
				  }
  		  	  	  sprintf(tx2buf,"\r\n%d %d",dataCnt,i);
